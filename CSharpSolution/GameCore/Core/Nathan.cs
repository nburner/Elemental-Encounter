    ...
    class Nathan : Player
	{
		public override move getMove(GameBoard gp)
        {
            ...
            do
            {
                ...
                } while (moveFrom != null && (moveFrom.Length != 2 || Char.ToUpper(moveFrom[0]) < 'A' || Char.ToUpper(moveFrom[0]) > 'H' || moveFrom[1] > '8' || moveFrom[1] < '1'));
                do
                {
                    Console.Write("Move To: ");
                    moveTo = Console.ReadLine();
                    //if (moveTo == "exit") throw 0;
                } while (moveTo != null && (moveTo.Length != 2 || Char.ToUpper(moveTo[0]) < 'A' || Char.ToUpper(moveTo[0]) > 'H' || moveTo[1] > '8' || moveTo[1] < '1'));

                if (moveFrom == null || moveTo == null) return new move(Square.A1, Square.A1);
